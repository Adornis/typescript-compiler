...
export default class CompileService {
  ...
  compile(filePath, moduleName) {
    ...
    _.each(
      result.outputFiles,
      function(file) {
        if (normalizePath(filePath) !== normalizePath(file.name)) return;
...
export function createCSResult(filePath, result) {
  const props = ['code', 'sourceMap', 'version', 'isExternal', 'dependencies', 'diagnostics'];
  ...
  result.diagnostics = new TsDiagnostics(result.diagnostics);
